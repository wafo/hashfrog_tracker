        ...
        <div className="check-tracker-locations">
          {locationsByType.map(loc => {
            ...
            if ((loc.available === 0 && loc.locked === 0) || loc.checked >= loc.checks.length) {
              ...
              if (loc.locked < 1 && loc.available > 0) style.borderLeftColor = "#198754";
              if (loc.locked > 0 && loc.available > 0) style.borderLeftColor = "#ffc107";
              if (loc.locked > 0 && loc.available < 1) style.borderLeftColor = "#dc3545";
            }
            return (
